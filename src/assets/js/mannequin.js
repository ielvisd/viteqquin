...
export const MANNEQUIN_VERSION = 4.02;
...
export function cossers(u, v, params) {
  function cosser(t, min, max) {
    if (t < min) t++;
    if (t > max) t--;
    if (min <= t && t <= max)
      return (
        0.5 + 0.5 * Math.cos(((t - min) / (max - min)) * 2 * Math.PI - Math.PI)
      );
    return 0;
  }
  for (var i = 0, r = 1; i < params.length; i++)
    r +=
      (cosser(u, params[i][0], params[i][1]) *
        cosser(v, params[i][2], params[i][3])) /
      params[i][4];
  return r;
}
    ...
    super(
      ...
      function (u, v, target) {
        var r = cossers(u, v, [
          ...
          [0, 1, 0, 0.1, 3],
          [0, 1, 0.9, 1, 3],
          [1.0, 1.05, 0.55, 0.85, -3],
          [1.0, 1.05, 0.15, 0.45, -3],
          [0.93, 1.08, 0.4, 0.6, 8],
          [0.0, 0.7, 0.05, 0.95, 3],
          [-0.2, 0.2, -0.15, 1.15, -6],
        ...
        function (u, v, target) {
          ...
          target.set(
            (3 * r - 2) *
              params[0] *
              (cos(u) * cos(v) +
                (feminine ? Math.pow(sin(u + 180), 2) * cos(v) - 1 : 0)) -
              (feminine ? 0 : 2),
    ...
    super(
      ...
      function (u, v, target) {
        var r = cossers(u, v, [
          ...
          [0.0, 0.3, 0.3, 0.9, feminine ? 1000 : 5],
          [-0.2, 0.3, 0, 0.3, -4],
          [-0.2, 0.3, -0.3, 0, -4]
        ]);
        u = 360 * u - 90;
...
export class Joint extends THREE.Group {
  ...
  point(x, y, z) {
    return scene.worldToLocal(
      this.children[0].localToWorld(new THREE.Vector3(x, y, z))
    );
  }
...
class Arm extends Joint {
  constructor(parentJoint, leftOrRight) {
    super(
      ...
      [-leftOrRight * 10, leftOrRight * 180, -leftOrRight * 180],
...
export class Mannequin extends Joint {
...
// head texture
Mannequin.texHead = new THREE.TextureLoader().load(
  "data:image/png;base64,"
);

// limb and body texture
Mannequin.texLimb = new THREE.TextureLoader().load(
  "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABAAQMAAACQp+OdAAAABlBMVEX////Ly8vsgL9iAAAAHElEQVQoz2OgEPyHAjgDjxoKGWTaRRkYDR/8AAAU9d8hJ6+ZxgAAAABJRU5ErkJggg=="
);
